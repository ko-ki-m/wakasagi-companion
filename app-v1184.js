...
function popup(g){
  ...
  const dateList=trips.map((t)=>{
    ...
    return `<button type="button" class="popupDateBtn" data-popup-trip-id="${esc(t.trip_id)}">${esc(label)}</button>`;
  }).join('');
  return `<div class="popupWrap" data-popup-group-id="${esc(g.group_id)}">
    <div class="popupTitle">${esc(title(g.latest))}</div>
    <div class="popupMeta">この場所の過去釣行日 ${g.count}回</div>
    <div class="popupDates"><b>見たい釣行日を選択</b>${dateList}</div>
    <div class="popupSelectedDetail"><div class="popupInlineDetail">日付をタップすると、その釣行回の詳細を表示します。</div></div>
  </div>`;
}
...
function v113_currentUrlWithoutAuto(){
  try{
    ...
    return location.origin + location.pathname + '?v=1184&linked=1&force=' + Date.now();
  }
}
try{
...
// ============================================================
// v11.8.3: select a trip date inside the marker popup.
// Popup first shows dates only; tapping a date shows details.
// ============================================================
