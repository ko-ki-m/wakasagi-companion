...
function fitInitialLakeViewOnce(force=false){if((initialLakeViewDone&&!force)||!map)return;const pts=[];const nearLimitM=1000;if(currentPos&&validLatLng(Number(currentPos.lat),Number(currentPos.lng)))pts.push([Number(currentPos.lat),Number(currentPos.lng)]);for(const g of groups){if(currentPos&&validLatLng(Number(currentPos.lat),Number(currentPos.lng))){const d=dist(Number(currentPos.lat),Number(currentPos.lng),g.lat,g.lng);if(d<=nearLimitM)pts.push([g.lat,g.lng]);}else{pts.push([g.lat,g.lng]);}}if(pts.length===0)return;if(pts.length===1)map.setView(pts[0],12);else map.fitBounds(L.latLngBounds(pts),{padding:[55,55],maxZoom:12});initialLakeViewDone=true;}
