...
function genId(p){
  return `${p}${Date.now().toString(36)}${Math.floor(Math.random()*0xfffff).toString(16).padStart(5,'0')}`;
}
function esc(v){
...
function sub(t){
  return `ライン ${esc(t.line_no||'-')} / シンカー ${esc(t.sinker_g||'-')}g / 魚探 ${esc(tripDepthText(t))} / 水温 ${esc(t.water_temp_c||'-')}℃`;
}
function dCurrent(t){
...
function dBase(t,a,b){
  if(!validLatLng(Number(a),Number(b))||!validLatLng(lat(t),lng(t)))return null;
...
function ensureMap(){
  ...
  if(!window.L){
    const s=$('mapStatus');
    if(s)s.textContent='地図ライブラリ未読込（GPS記録は可）';
    return false;
  }
...
function drawCurrent(zoomNow=false){
  ...
  const ids=['btnFitNear','btnSaveScroll','btnSaveTrip'];
...
function popup(g){
  ...
  const dates=trips.map(t=>`<a class="popupBtn" href="#" data-popup-group-id="${esc(g.group_id)}" data-popup-trip-id="${esc(t.trip_id)}">${esc(fmtTime(tms(t)).split(' ')[0])}</a>`).join(' ');
  return`<div class="popupTitle">この地点の過去 ${g.count}回</div><div class="popupMeta">見たい日付を選択してください。</div>${dates||'<div class="popupMeta">履歴がありません。</div>'}`;
}
function ensureFrontDetailBox(){
  let o=document.getElementById('frontTripDetailOverlay');
  if(o)return o;
  o=document.createElement('div');
  o.id='frontTripDetailOverlay';
  o.style.cssText='display:none;position:fixed;z-index:99999;left:10px;right:10px;top:10px;max-height:86vh;overflow:auto;background:#fff;color:#0f172a;border:3px solid #0b84ff;border-radius:18px;box-shadow:0 18px 60px rgba(0,0,0,.45);padding:14px;';
  document.body.appendChild(o);
  return o;
}
function closeFrontTripDetail(){
  const o=document.getElementById('frontTripDetailOverlay');
  if(o)o.style.display='none';
}
function showFrontTripDetail(t,base=null){
  const o=ensureFrontDetailBox();
  o.innerHTML=`<div class="cardHead"><div><h2>${esc(fmtTime(tms(t)).split(' ')[0])}　${esc(title(t))}</h2><p>この釣行回の詳細</p></div><button id="frontDetailClose" type="button">閉じる</button></div><div class="summaryBox" style="margin-top:10px">${detailHtml(t,base)}</div>`;
...
function updatePosition(pos,zoomNow=false,doInitialFit=true){
  currentPos={lat:Number(pos.lat),lng:Number(pos.lng),acc:Number(pos.acc||0),t:Number(pos.t||nowMs())};
  if($('latView'))$('latView').textContent=currentPos.lat.toFixed(7);
  if($('lngView'))$('lngView').textContent=currentPos.lng.toFixed(7);
  if($('accView'))$('accView').textContent=currentPos.acc?`±${Math.round(currentPos.acc)}m`:'-';
  if($('timeView'))$('timeView').textContent=fmtTime(currentPos.t);
  if($('locStatus'))$('locStatus').textContent='現在地を確認しました。';
  setBadge('locBadge','取得済み',currentPos.acc>0&&currentPos.acc<=20?'good':'warn');
  metaSet('last_pos',currentPos);
  drawCurrent(zoomNow);
  refreshAll().then(()=>{if(doInitialFit)fitInitialLakeViewOnce(true);});
}
function locate(manual=false){
  if(!window.isSecureContext){
    if($('locStatus'))$('locStatus').textContent='HTTPSで開いていません。GitHub Pagesのhttps URLから開いてください。';
  ...
  if($('locStatus'))$('locStatus').textContent='現在地を取得しています...';
  setBadge('locBadge','取得中','warn');
  navigator.geolocation.getCurrentPosition(g=>{
    const c=g.coords;
    updatePosition({lat:Number(c.latitude),lng:Number(c.longitude),acc:Number(c.accuracy||0),t:Number(g.timestamp||nowMs())},manual,!manual);
  },async e=>{
    if($('locStatus'))$('locStatus').textContent='現在地エラー: '+(e&&e.message?e.message:'取得できませんでした');
    setBadge('locBadge','未取得','bad');
    const last=await metaGet('last_pos');
    if(last&&validLatLng(Number(last.lat),Number(last.lng))){
      if($('locStatus'))$('locStatus').textContent='前回位置を表示しています。';
...
function readForm(){
  return{
    date_ms:fromLocal($('tripDate')?$('tripDate').value:''),
    lake_name:$('lakeName')?$('lakeName').value.trim():'',
    point_name:$('pointName')?$('pointName').value.trim():'',
    line_no:$('lineNo')?$('lineNo').value.trim():'',
    sinker_g:$('sinkerG')?$('sinkerG').value.trim():'',
    fishfinder_depth_m:$('fishfinderDepthM')?$('fishfinderDepthM').value.trim():'',
    water_temp_c:$('waterTempC')?$('waterTempC').value.trim():'',
    weather:$('weather')?$('weather').value.trim():'',
    wind:$('wind')?$('wind').value.trim():'',
    memo:$('memo')?$('memo').value.trim():''
  };
}
function fillForm(t){
  if($('tripDate'))$('tripDate').value=toLocal(tms(t));
  if($('lakeName'))$('lakeName').value=t.lake_name||'';
  if($('pointName'))$('pointName').value=t.point_name||'';
  if($('lineNo'))$('lineNo').value=t.line_no||'';
  if($('sinkerG'))$('sinkerG').value=t.sinker_g||'';
  if($('fishfinderDepthM'))$('fishfinderDepthM').value=t.fishfinder_depth_m||'';
  if($('waterTempC'))$('waterTempC').value=t.water_temp_c||'';
  if($('weather'))$('weather').value=t.weather||'';
  if($('wind'))$('wind').value=t.wind||'';
  if($('memo'))$('memo').value=t.memo||'';
}
function clearForm(){
  editingTripId=null;
  if($('tripDate'))$('tripDate').value=toLocal(nowMs());
  ['lakeName','pointName','lineNo','sinkerG','fishfinderDepthM','waterTempC','weather','wind','memo'].forEach(id=>{if($(id))$(id).value='';});
  if($('btnUpdateTrip'))$('btnUpdateTrip').disabled=true;
...
async function saveTrip(){
  ...
  const trips=await getAllTrips();
  const near=trips.filter(t=>dBase(t,currentPos.lat,currentPos.lng)!==null&&dBase(t,currentPos.lat,currentPos.lng)<=SAME_POINT_M);
...
function drawSelected(a,b){
  ...
  sel20=L.circle([a,b],{radius:SAME_POINT_M,weight:3,fillOpacity:.045}).addTo(map);
  sel100=L.circle([a,b],{radius:SAME_AREA_M,weight:2,fillOpacity:.018}).addTo(map);
}
function showTripDetail(t,base=null){
  selectedTripId=t.trip_id;
  if($('selectedMini'))$('selectedMini').textContent=title(t);
  if($('selectedLead'))$('selectedLead').innerHTML=`${esc(title(t))}<br><span class="muted">${esc(fmtTime(tms(t)))}</span>`;
  setBadge('selectedBadge','詳細表示中','good');
  if(!$('selectedTripDetail'))return;
  $('selectedTripDetail').className='detail';
  $('selectedTripDetail').innerHTML=`<div class="summaryBox"><h3>選択した釣行回</h3>${detailHtml(t,base)}</div><div class="actions"><button id="btnStdSelected">標準地図</button><button id="btnEditSelected">入力欄へ読込</button><button id="btnDeleteSelected" class="danger">この履歴を削除</button></div>`;
  if($('btnStdSelected'))$('btnStdSelected').onclick=()=>openStd(lat(t),lng(t));
  if($('btnEditSelected'))$('btnEditSelected').onclick=()=>loadToForm(t.trip_id);
  if($('btnDeleteSelected'))$('btnDeleteSelected').onclick=async()=>{
    ...
    if($('selectedLead'))$('selectedLead').textContent='地図の数字ピン、または下の全履歴から1回分を選ぶと詳細を表示します。';
    setBadge('selectedBadge','未選択','');
    await refreshAll();
  };
}
async function hidePointHistoryCard(){
  const p=$('pointHistoryPanel');
  if(!p)return;
  const c=p.closest('.card');
  if(c)c.style.display='none';
}
function hideSelectedTripDetailCard(){
  const p=$('selectedTripDetail');
  if(!p)return;
  const c=p.closest('.card');
  if(c)c.style.display='none';
}
async function refreshCounts(){
  const trips=await getAllTrips();
  if($('totalTrips'))$('totalTrips').textContent=String(trips.length);
  if(!currentPos){
    if($('count20'))$('count20').textContent='-';
    if($('count100'))$('count100').textContent='-';
    return;
  }
  const arr=trips.map(t=>dCurrent(t)).filter(d=>d!==null);
  if($('count20'))$('count20').textContent=String(arr.filter(d=>d<=SAME_POINT_M).length);
  if($('count100'))$('count100').textContent=String(arr.filter(d=>d<=SAME_AREA_M).length);
}
function filterTrips(trips){
  const q=($('searchBox')?$('searchBox').value:'').trim().toLowerCase();
  let list=trips.slice();
  if(q){
    list=list.filter(t=>[fmtTime(tms(t)),t.lake_name,t.point_name,t.line_no,t.sinker_g,t.fishfinder_depth_m,t.water_temp_c,t.weather,t.wind,t.memo].join(' ').toLowerCase().includes(q));
  }
  const m=$('sortMode')?$('sortMode').value:'date_desc';
  list.sort((a,b)=>{
...
async function renderAllList(){
  ...
  if($('dbView'))$('dbView').textContent=`過去釣行履歴 ${trips.length}件`;
...
async function updateDb(){
  const trips=await getAllTrips();
  groups=makeGroups(trips);
  if($('dbView'))$('dbView').textContent=`過去釣行履歴 ${trips.length}件 / 地図ポイント ${groups.length}件`;
  setBadge('groupView',`地図${groups.length}`,'good');
  await renderAllList();
}
...
function loadToForm(id){
  getAllTrips().then(trips=>{
    ...
    editingTripId=id;
    fillForm(t);
    if($('btnUpdateTrip'))$('btnUpdateTrip').disabled=false;
    if($('btnLoadSelected'))$('btnLoadSelected').disabled=false;
    setBadge('saveBadge','編集中','warn');
    if($('tripDate'))$('tripDate').scrollIntoView({behavior:'smooth',block:'center'});
  });
}
function openStd(a,b){
  window.open(`https://www.google.com/maps/search/?api=1&query=${Number(a).toFixed(7)},${Number(b).toFixed(7)}`,'_blank','noopener');
}
function fitAll(){
  if(!map)return;
  const pts=[];
...
function fitInitialLakeViewOnce(force=false){
  if((initialLakeViewDone&&!force)||!map)return;
  const nearLimitM=1000;
  const pts=[];
  if(currentPos&&validLatLng(Number(currentPos.lat),Number(currentPos.lng))){
    const a=Number(currentPos.lat),b=Number(currentPos.lng);
    const dLat=nearLimitM/111320;
    const dLng=nearLimitM/(111320*Math.max(0.25,Math.cos(a*Math.PI/180)));
    pts.push([a-dLat,b-dLng],[a+dLat,b+dLng]);
    for(const g of groups){
      const d=dist(a,b,g.lat,g.lng);
      if(d<=nearLimitM)pts.push([g.lat,g.lng]);
    }
  }else{
    for(const g of groups)pts.push([g.lat,g.lng]);
  }
  ...
  initialLakeViewDone=true;
}
function fitNear(){
  if(!map||!currentPos)return;
  const pts=[[Number(currentPos.lat),Number(currentPos.lng)]];
  for(const g of groups){
    if(g.distance_m!==null&&g.distance_m<=SAME_AREA_M)pts.push([g.lat,g.lng]);
  }
  if(pts.length===1)map.setView(pts[0],18);
  else map.fitBounds(L.latLngBounds(pts),{padding:[35,35],maxZoom:18});
}
function downloadBlob(n,t,x){
  const b=new Blob([x],{type:t});
...
async function exportDb(){
  const trips=await getAllTrips();
  downloadBlob(`wakasagi_map_v10_${Date.now()}.json`,'application/json',JSON.stringify({version:'10',exported_ms:Date.now(),trips},null,2));
}
...
async function initPwa(){
  if('serviceWorker'in navigator){
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=restore_history_20260514');
      try{await reg.update();}catch(e){}
    }catch(e){}
  }
}
async function init(){
  hidePointHistoryCard();
  hideSelectedTripDetailCard();
  if(window.isSecureContext)setBadge('secureBadge','GPS可','good');
  else setBadge('secureBadge','HTTPS必要','bad');
  db=await openDb();
  ensureMap();
  if($('tripDate'))$('tripDate').value=toLocal(nowMs());
  const n=await migrateOld(false);
  if(n>0&&$('mapStatus'))$('mapStatus').textContent=`旧データ${n}件を移行しました。`;

  if($('btnLocate'))$('btnLocate').onclick=()=>locate(true);
  if($('btnFitAll'))$('btnFitAll').onclick=fitAll;
  if($('btnFitNear'))$('btnFitNear').onclick=fitNear;
  if($('btnSaveScroll'))$('btnSaveScroll').onclick=()=>$('tripDate')&&$('tripDate').scrollIntoView({behavior:'smooth',block:'center'});
  if($('btnSaveTrip'))$('btnSaveTrip').onclick=saveTrip;
  if($('btnLoadSelected'))$('btnLoadSelected').onclick=()=>selectedTripId&&loadToForm(selectedTripId);
  if($('btnUpdateTrip'))$('btnUpdateTrip').onclick=updateTrip;
  if($('btnClearForm'))$('btnClearForm').onclick=clearForm;
  if($('btnExport'))$('btnExport').onclick=exportDb;
  if($('btnImport'))$('btnImport').onclick=()=>$('importFile')&&$('importFile').click();
  if($('importFile'))$('importFile').onchange=async e=>{
    const f=e.target.files&&e.target.files[0];
    if(!f)return;
    let p;
    try{p=JSON.parse(await f.text());}catch(err){alert('JSONを読めません。');return;}
    const c=await importPayload(p);
    alert(`${c}件を読み込みました。`);
    await refreshAll();
  };
  if($('btnMigrate'))$('btnMigrate').onclick=async()=>{
    const c=await migrateOld(true);
    alert(`旧データ移行 ${c}件`);
    await refreshAll();
  };
  if($('btnClearDb'))$('btnClearDb').onclick=async()=>{
    if(confirm('テストDBを消去しますか？')){
  ...
  if($('searchBox'))$('searchBox').oninput=renderAllList;
  if($('sortMode'))$('sortMode').onchange=renderAllList;

  document.addEventListener('click',ev=>{
    const pd=ev.target.closest('[data-popup-trip-id]');
    if(pd){
      ev.preventDefault();
      showPopupTripDetail(pd.getAttribute('data-popup-group-id'),pd.getAttribute('data-popup-trip-id'),null);
      return;
    }
    const all=ev.target.closest('[data-all-date-trip-id]');
    if(all){
      ev.preventDefault();
      selectTrip(all.getAttribute('data-all-date-trip-id'));
      return;
    }
    const item=ev.target.closest('[data-trip-id]');
    if(item){
      ev.preventDefault();
      selectTrip(item.getAttribute('data-trip-id'));
      return;
    }
  });

  const last=await metaGet('last_pos');
  if(last&&validLatLng(Number(last.lat),Number(last.lng)))updatePosition(last,false,true);
  else locate(false);
...
// ============================================================
// v11.1: GitHub map -> Pico W /log link
// ============================================================
function v11_utf8_to_b64(str){
  return btoa(unescape(encodeURIComponent(str)));
}
function v11_b64_to_utf8(str){
  return decodeURIComponent(escape(atob(str)));
}
function v11_getPicoIp(){
  try{
    const el=document.getElementById('picoIp');
    let v=el&&el.value?el.value:localStorage.getItem('pico_ip');
    if(!v)v='192.168.4.1';
    v=String(v).replace(/^https?:\/\//,'').replace(/\/.*$/,'').trim();
    return v||'192.168.4.1';
  }catch(e){
    return '192.168.4.1';
  }
}
...
function v11_enableLinkButton(){
  ...
  btn.disabled=!ok;
  if(ok) v11_setLinkStatus('現在地または選択地点をPico Wの現在sidへ記録できます。');
}
...
function v112_decodeLogSyncPayload(){
  try{
    const p=new URLSearchParams(location.search);
    let raw=p.get('logsync') || p.get('pico_log') || p.get('payload');
    if(!raw && location.hash){
      const h=new URLSearchParams(location.hash.replace(/^#/,''));
      raw=h.get('logsync') || h.get('pico_log') || h.get('payload');
    }
    if(!raw)return null;
    return JSON.parse(v11_b64_to_utf8(decodeURIComponent(raw)));
  }catch(e){
    return {__error:String(e)};
  }
}
function v112_logSummaryHtml(t){
  const s=t&&t.pico_summary;
  if(!s)return '';
  const val=v=>v===undefined||v===null||v===''?'-':String(v);
  return `<div class="summaryBox"><h3>Pico Wログ要約</h3><div class="kv">
    <b>sid</b><span>${esc(val(s.sid))}</span>
    <b>魚数</b><span>${esc(val(s.fish_count))}</span>
    <b>マーク</b><span>${esc(val(s.mark_count))}</span>
    <b>ログ数</b><span>${esc(val(s.tlog_count))}</span>
    <b>seq</b><span>${esc(val(s.first_seq))} - ${esc(val(s.last_seq))}</span>
    <b>受信</b><span>${esc(fmtTime(s.received_ms||s.updated_ms||s.last_recv_ms))}</span>
...
// 復旧の最重要点:
// point_visit_id / map_point_key / map_spot_id では既存履歴を探さない。
// 同じPico W起動中のsidだけを既存更新対象にする。
// これにより、本体電源を入れ直した後は同じ地点でも新しい履歴として増える。
...
async function v112_applyLogSyncPayload(p){
  ...
  t.pico_sid = sid;
  if(!t.point_visit_id && pointKey) t.point_visit_id = pointKey;
  if(!t.map_point_key && pointKey) t.map_point_key = pointKey;
...
async function v1155_checkOfflineReady(){
  try{
    ...
    if(reg){
      v1155_setReadyStatus('GPS連携：準備済み。現在地を記録できます。','good');
      return true;
    }
  }catch(e){}
  v1155_setReadyStatus('GPS連携：準備中です。インターネット接続中に一度この画面を開くと準備されます。','warn');
  return false;
}
...
function v116_setOfflineGpsNotice(){
  try{
    ...
    if(auto && noLeaflet){
      ...
      if(m) m.textContent='GPS連携モード：地図タイルなしで現在地だけ取得します。';
    }else if(noLeaflet){
      const m=document.getElementById('mapStatus');
