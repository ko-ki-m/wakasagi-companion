...
function fitInitialLakeViewOnce(){if(initialLakeViewDone||!map)return;const pts=[];const nearLimitM=1000;if(currentPos&&validLatLng(Number(currentPos.lat),Number(currentPos.lng)))pts.push([Number(currentPos.lat),Number(currentPos.lng)]);for(const g of groups){if(currentPos&&validLatLng(Number(currentPos.lat),Number(currentPos.lng))){const d=dist(Number(currentPos.lat),Number(currentPos.lng),g.lat,g.lng);if(d<=nearLimitM)pts.push([g.lat,g.lng]);}else{pts.push([g.lat,g.lng]);}}if(pts.length===0)return;if(pts.length===1)map.setView(pts[0],12);else map.fitBounds(L.latLngBounds(pts),{padding:[55,55],maxZoom:12});initialLakeViewDone=true;}
...
async function init(){hidePointHistoryCard();if(window.isSecureContext)setBadge('secureBadge','GPS可','good');else setBadge('secureBadge','HTTPS必要','bad');db=await openDb();ensureMap();$('tripDate').value=toLocal(nowMs());const n=await migrateOld(false);if(n>0)$('mapStatus').textContent=`旧データ${n}件を移行しました。`;
...
});await initPwa();await refreshAll();const last=await metaGet('last_pos');if(last&&validLatLng(Number(last.lat),Number(last.lng)))updatePosition(last,false,false);locate(false);}
