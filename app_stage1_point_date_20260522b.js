...
function stage1DateLabel(key){
  if(!key || key==='unknown') return '日付不明';
  return String(key).replace(/-/g,'/');
}
function stage1TimeLabel(t){
  const s=fmtTime(tms(t));
  const parts=String(s).split(' ');
  return parts.length>1 ? parts[1] : s;
}
function stage1DateGroups(trips){
  const m=new Map();
  for(const t of (trips||[])){
    const key=stage1DateKey(t);
    if(!m.has(key)) m.set(key,{key,label:stage1DateLabel(key),trips:[],latest_ms:0});
    const g=m.get(key);
    g.trips.push(t);
    const ms=tms(t);
    if(ms>g.latest_ms) g.latest_ms=ms;
  }
  const arr=[...m.values()];
  for(const g of arr) g.trips.sort((a,b)=>tms(b)-tms(a));
...
function makeGroups(trips){
  ...
  for(const g of gs){
    ...
    g.date_groups=stage1DateGroups(g.trips);
    g.date_count=g.date_groups.length;
    g.count=g.date_count;
...
function popup(g){
  ...
  const days=stage1DateGroups(trips);
  const dates=days.map(d=>{
    const label=d.trips.length>1 ? `${d.label} (${d.trips.length})` : d.label;
    return `<a class="popupBtn" href="#" data-popup-group-id="${esc(g.group_id)}" data-popup-date-key="${esc(d.key)}">${esc(label)}</a>`;
  }).join(' ');
  const tripCount=trips.length;
  const dayCount=days.length;
  const suffix=tripCount!==dayCount ? ` / ${tripCount}件` : '';
  return`<div class="popupTitle">この地点の過去 ${dayCount}日${suffix}</div><div class="popupMeta">見たい日付を選択してください。同じ日付が複数ある場合は時刻一覧を表示します。</div>${dates||'<div class="popupMeta">履歴がありません。</div>'}`;
}
...
function showPopupDateList(groupId,box){
  const g=(groups||[]).find(x=>String(x.group_id)===String(groupId));
  if(g&&box)box.innerHTML=popup(g);
}
async function showPopupDateTrips(groupId,dateKey,box){
  const g=(groups||[]).find(x=>String(x.group_id)===String(groupId));
  if(!g)return;
  const rows=(g.trips||[]).filter(t=>stage1DateKey(t)===String(dateKey)).sort((a,b)=>tms(b)-tms(a));
  if(rows.length<=1){
    if(rows[0]) await showPopupTripDetail(groupId,rows[0].trip_id,box);
    return;
  }
  if(!box)return;
  const dateLabel=stage1DateLabel(dateKey);
  const buttons=rows.map(t=>`<a class="popupBtn" href="#" data-popup-group-id="${esc(groupId)}" data-popup-trip-id="${esc(t.trip_id)}">${esc(stage1TimeLabel(t))}　${esc(title(t))}</a>`).join(' ');
  box.innerHTML=`<div class="popupTitle">${esc(dateLabel)} の履歴 ${rows.length}件</div><div class="popupMeta">見たい時刻を選択してください。</div>${buttons}<div class="popupMeta"><a class="popupBtn" href="#" data-popup-back-group-id="${esc(groupId)}">日付一覧へ戻る</a></div>`;
}
...
$('btnLocate').onclick=()=>locate(true);$('btnFitAll').onclick=fitAll;$('btnFitNear').onclick=fitNear;$('btnSaveScroll').onclick=()=>$('tripDate').scrollIntoView({behavior:'smooth',block:'center'});$('btnSaveTrip').onclick=saveTrip;$('btnLoadSelected').onclick=()=>selectedTripId&&loadToForm(selectedTripId);$('btnUpdateTrip').onclick=updateTrip;$('btnClearForm').onclick=clearForm;$('btnExport').onclick=exportDb;$('btnImport').onclick=()=>$('importFile').click();$('importFile').onchange=async e=>{const f=e.target.files&&e.target.files[0];if(!f)return;let p;try{p=JSON.parse(await f.text());}catch(err){alert('JSONを読めません。');return;}const c=await importPayload(p);alert(`${c}件を読み込みました。`);await refreshAll();};$('btnMigrate').onclick=async()=>{const c=await migrateOld(true);alert(`旧データ移行 ${c}件`);await refreshAll();};$('btnClearDb').onclick=async()=>{if(confirm('テストDBを消去しますか？')){await new Promise(res=>{const tx=db.transaction([STORE_TRIPS,STORE_META],'readwrite');tx.objectStore(STORE_TRIPS).clear();tx.objectStore(STORE_META).clear();tx.oncomplete=()=>res();});location.reload();}};$('searchBox').oninput=renderAllList;$('sortMode').onchange=renderAllList;document.addEventListener('click',ev=>{
  ...
  const pk=ev.target.closest('[data-popup-date-key]');
  if(pk){
    ev.preventDefault();ev.stopPropagation();
    const box=pk.closest('.leaflet-popup-content');
    showPopupDateTrips(pk.getAttribute('data-popup-group-id'),pk.getAttribute('data-popup-date-key'),box);
    return;
  }
