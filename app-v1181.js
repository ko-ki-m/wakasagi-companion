...
function popupTripMini(t, base){
  ...
  return `<div class="popupInlineDetail"><div class="kvMini">
    <b>日付</b><span>${esc(fmtTime(tms(t)))}</span>
    <b>距離</b><span>${esc(dd)}</span>
    <b>ライン</b><span>${esc(t.line_no||'-')}</span>
    <b>シンカー</b><span>${esc(t.sinker_g||'-')}g</span>
    <b>魚探</b><span>${esc(t.fishfinder_depth_m||t.fishfinder_m||'-')}m</span>
    <b>水温</b><span>${esc(t.water_temp_c||'-')}℃</span>
    <b>メモ</b><span>${esc(t.memo||'-')}</span>
  </div>${log}</div>`;
}
function popup(g){
  const dd=g.distance_m==null?'':`<br>現在地から ${Math.round(g.distance_m)}m`;
  const trips=(g.trips||[]).slice().sort((a,b)=>tms(b)-tms(a));
  const dateList=trips.map((t,i)=>{
    const label=fmtTime(tms(t));
    const line=[t.line_no?`L${t.line_no}`:'',t.sinker_g?`${t.sinker_g}g`:'',t.fishfinder_depth_m?`${t.fishfinder_depth_m}m`:''].filter(Boolean).join(' / ');
    return `<button type="button" class="popupDateBtn${i===0?' active':''}" data-popup-trip-id="${esc(t.trip_id)}">${esc(label)}${line?`<br><span>${esc(line)}</span>`:''}</button>`;
  }).join('');
  const chooser=trips.length>1
    ? `<div class="popupDates"><b>釣行日を選択</b>${dateList}</div>`
    : `<div class="popupDates"><b>釣行日</b>${dateList}</div>`;
  return `<div class="popupWrap" data-popup-group-id="${esc(g.group_id)}">
    <div class="popupTitle">${esc(title(g.latest))}</div>
    <div class="popupMeta">この地点の過去 ${g.count}回${dd}</div>
    ${chooser}
    <div class="popupSelectedDetail">${popupTripMini(g.latest,{lat:g.lat,lng:g.lng})}</div>
  </div>`;
}
function showGroupNoRedraw(g){
  ...
  try{ renderPointHistory(g,selectedTripId); }catch(e){}
  try{ showTripDetail(g.latest,{lat:g.lat,lng:g.lng}); }catch(e){}
  try{ drawSelected(g.lat,g.lng); }catch(e){}
}
async function renderMap(){
  ...
  for(const g of groups){
    ...
    L.marker([g.lat,g.lng],{icon:ic})
      .addTo(groupLayer)
      .bindPopup(popup(g),{maxWidth:320,minWidth:250,autoPan:true,closeButton:true})
      .on('click',()=>showGroupNoRedraw(g));
  }
}
...
async function initPwa(){ /* service worker disabled in v11.8.1 */ }catch(e){}}}
...
function v113_currentUrlWithoutAuto(){
  try{
    ...
    return location.origin + location.pathname + '?v=1181&linked=1&force=' + Date.now();
  }
}
try{
...
// ============================================================
// v11.8: select a trip date inside the marker popup.
// No redraw on marker tap, because redraw closes the popup.
// ============================================================
async function v118_popupSelectTrip(tripId, btn){
  try{
    const trips=await getAllTrips();
    const t=trips.find(x=>String(x.trip_id)===String(tripId));
    if(!t) return;
    selectedTripId=t.trip_id;
    const g=(groups||[]).find(gr=>Array.isArray(gr.trips)&&gr.trips.some(x=>String(x.trip_id)===String(tripId)));
    if(g){
      selectedGroupId=g.group_id;
      renderPointHistory(g,t.trip_id);
      showTripDetail(t,{lat:g.lat,lng:g.lng});
      drawSelected(g.lat,g.lng);
    }else{
      showTripDetail(t,null);
    }
    const wrap=btn.closest('.popupWrap');
    if(wrap){
      wrap.querySelectorAll('.popupDateBtn').forEach(x=>x.classList.remove('active'));
      btn.classList.add('active');
      const detail=wrap.querySelector('.popupSelectedDetail');
      if(detail) detail.innerHTML=popupTripMini(t,g?{lat:g.lat,lng:g.lng}:null);
    }
  }catch(e){}
}
document.addEventListener('click',function(ev){
  const b=ev.target.closest('.popupDateBtn');
  if(!b) return;
  ev.preventDefault();
  ev.stopPropagation();
  v118_popupSelectTrip(b.getAttribute('data-popup-trip-id'), b);
}, true);
