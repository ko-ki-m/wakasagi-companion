'use strict';
(function(){
  const PATCH = 'logsync_commit_ack_20260607a';
  ...
  async function afterOriginalApply(p, ret){
    ...
    const v = await verifySaved(p);
    if(!v.ok){
      setStatus('Map保存未確認: 再試行待ち','bad');
      console.warn('[wakasagi] '+PATCH+' verify failed', v, p);
      return false;
    }
