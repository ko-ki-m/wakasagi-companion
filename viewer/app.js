...
function makeLakeDebugTrip(){
  const now = Date.now();
  return {
    trip_id: 'TEST_NOJIRI_LAKE_SMARTPHONE_001',
    date_ms: now,
    start_ms: now,
    end_ms: now + 3600000,
    lat: 36.8322,
    ...
    accuracy_m: 5,
    lake_name: '',
    point_name: 'スマホ湖名推定テスト地点',
    line_no: '0.2',
    sinker_g: 15,
    fishfinder_depth_m: 35.2,
    water_temp_c: 4.2,
    weather: '',
    wind: '',
    memo: 'スマホChrome側DBで湖名推定を確認するための一時テストデータ。確認後に削除してよい。',
    pico_summary: {
      fish_count: 0,
      mark_count: 0,
      log_count: 0,
...
async function putDebugTrip(){
  if(!db) db = await openDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_TRIPS, 'readwrite');
    tx.objectStore(STORE_TRIPS).put(makeLakeDebugTrip());
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error);
  });
  g_lakeGuessCache.clear();
  selectedTripId = 'TEST_NOJIRI_LAKE_SMARTPHONE_001';
  await reload();
  const t = findTrip('TEST_NOJIRI_LAKE_SMARTPHONE_001');
  if(t) selectTrip(t.trip_id);
  $('statusText').textContent = '野尻湖テストデータを追加しました。湖名が表示されるか確認してください。';
}

async function deleteDebugTrip(){
  if(!db) db = await openDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_TRIPS, 'readwrite');
    tx.objectStore(STORE_TRIPS).delete('TEST_NOJIRI_LAKE_SMARTPHONE_001');
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error);
  });
  if(selectedTripId === 'TEST_NOJIRI_LAKE_SMARTPHONE_001'){
    selectedTripId = null;
    $('detailCard').classList.add('hidden');
    $('mapCard').classList.add('hidden');
  }
  g_lakeGuessCache.delete('TEST_NOJIRI_LAKE_SMARTPHONE_001');
  await reload();
  $('statusText').textContent = '野尻湖テストデータを削除しました。';
}

function addLakeDebugButtons(){
  const actions = document.querySelector('.actions');
  if(!actions || document.getElementById('btnAddLakeDebug')) return;

  const addBtn = document.createElement('button');
  addBtn.id = 'btnAddLakeDebug';
  addBtn.type = 'button';
  addBtn.textContent = '湖名テスト追加';
  addBtn.className = 'debugButton';

  const delBtn = document.createElement('button');
  delBtn.id = 'btnDeleteLakeDebug';
  delBtn.type = 'button';
  delBtn.textContent = 'テスト削除';
  delBtn.className = 'debugButton danger';

  addBtn.addEventListener('click', () => {
    putDebugTrip().catch(e => {
      console.error(e);
      $('statusText').textContent = '湖名テスト追加に失敗しました: ' + e;
    });
  });

  delBtn.addEventListener('click', () => {
    deleteDebugTrip().catch(e => {
      console.error(e);
      $('statusText').textContent = '湖名テスト削除に失敗しました: ' + e;
    });
  });
