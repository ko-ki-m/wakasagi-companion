...
function stage1DateKey(ms){return fmtTime(ms).split(' ')[0];}
function stage1TimeText(ms){const s=fmtTime(ms);return s.indexOf(' ')>=0?s.split(' ')[1]:s;}
function stage1DateGroups(trips){const m=new Map();for(const t of (trips||[])){const k=stage1DateKey(tms(t));if(!m.has(k))m.set(k,[]);m.get(k).push(t);}const out=[];for(const [date,rows] of m.entries()){rows.sort((a,b)=>tms(b)-tms(a));out.push({date,trips:rows,latest:rows[0],count:rows.length});}out.sort((a,b)=>tms(b.latest)-tms(a.latest));return out;}
function makeGroups(trips){const valid=trips.filter(x=>validLatLng(lat(x),lng(x))).slice().sort((a,b)=>tms(b)-tms(a));const gs=[];for(const t of valid){let best=null,bd=Infinity;for(const g of gs){const dd=dist(lat(t),lng(t),g.lat,g.lng);if(dd<=SAME_POINT_M&&dd<bd){best=g;bd=dd;}}if(best){best.trips.push(t);const n=best.trips.length;best.lat=(best.lat*(n-1)+lat(t))/n;best.lng=(best.lng*(n-1)+lng(t))/n;}else gs.push({group_id:'G'+gs.length+'_'+t.trip_id,lat:lat(t),lng:lng(t),trips:[t]});}for(const g of gs){g.trips.sort((a,b)=>tms(b)-tms(a));g.latest=g.trips[0];g.trip_count=g.trips.length;g.date_count=stage1DateGroups(g.trips).length;g.count=g.date_count;g.distance_m=currentPos?dist(Number(currentPos.lat),Number(currentPos.lng),g.lat,g.lng):null;g.latest_ms=tms(g.latest);}return gs.sort((a,b)=>currentPos?((a.distance_m??Infinity)-(b.distance_m??Infinity)):(b.latest_ms-a.latest_ms));}
...
function popupDateList(g){const dg=stage1DateGroups(g.trips||[]);const items=dg.map(d=>{const label=d.count>1?`${d.date} (${d.count})`:d.date;if(d.count===1){return `<a class="popupBtn" href="#" data-popup-group-id="${esc(g.group_id)}" data-popup-trip-id="${esc(d.latest.trip_id)}">${esc(label)}</a>`;}return `<a class="popupBtn" href="#" data-popup-group-id="${esc(g.group_id)}" data-popup-date-key="${esc(d.date)}">${esc(label)}</a>`;}).join(' ');return`<div class="popupTitle">この地点の過去 ${g.date_count||dg.length}日 / ${g.trip_count||(g.trips||[]).length}回</div><div class="popupMeta">見たい日付を選択してください。同日に複数回ある日は時刻一覧を表示します。</div>${items||'<div class="popupMeta">履歴がありません。</div>'}`;}
function popupTimeList(g,dateKey){const trips=(g.trips||[]).filter(t=>stage1DateKey(tms(t))===String(dateKey)).sort((a,b)=>tms(b)-tms(a));const items=trips.map(t=>`<a class="popupBtn" href="#" data-popup-group-id="${esc(g.group_id)}" data-popup-trip-id="${esc(t.trip_id)}">${esc(stage1TimeText(tms(t)))}　${esc(title(t))}</a>`).join(' ');return`<div class="popupTitle">${esc(dateKey)} の履歴 ${trips.length}回</div><div class="popupMeta"><a href="#" data-popup-group-id="${esc(g.group_id)}" data-popup-back-dates="1">日付一覧へ戻る</a></div>${items||'<div class="popupMeta">履歴がありません。</div>'}`;}
function popup(g){return popupDateList(g);}
...
function showPopupDateList(groupId,box){const g=(groups||[]).find(x=>String(x.group_id)===String(groupId));if(g&&box)box.innerHTML=popupDateList(g);}
...
$('btnLocate').onclick=()=>locate(true);$('btnFitAll').onclick=fitAll;$('btnFitNear').onclick=fitNear;$('btnSaveScroll').onclick=()=>$('tripDate').scrollIntoView({behavior:'smooth',block:'center'});$('btnSaveTrip').onclick=saveTrip;$('btnLoadSelected').onclick=()=>selectedTripId&&loadToForm(selectedTripId);$('btnUpdateTrip').onclick=updateTrip;$('btnClearForm').onclick=clearForm;$('btnExport').onclick=exportDb;$('btnImport').onclick=()=>$('importFile').click();$('importFile').onchange=async e=>{const f=e.target.files&&e.target.files[0];if(!f)return;let p;try{p=JSON.parse(await f.text());}catch(err){alert('JSONを読めません。');return;}const c=await importPayload(p);alert(`${c}件を読み込みました。`);await refreshAll();};$('btnMigrate').onclick=async()=>{const c=await migrateOld(true);alert(`旧データ移行 ${c}件`);await refreshAll();};$('btnClearDb').onclick=async()=>{if(confirm('テストDBを消去しますか？')){await new Promise(res=>{const tx=db.transaction([STORE_TRIPS,STORE_META],'readwrite');tx.objectStore(STORE_TRIPS).clear();tx.objectStore(STORE_META).clear();tx.oncomplete=()=>res();});location.reload();}};$('searchBox').oninput=renderAllList;$('sortMode').onchange=renderAllList;document.addEventListener('click',ev=>{
  const back=ev.target.closest('[data-popup-back-dates]');
  if(back){
    ev.preventDefault();ev.stopPropagation();
    const box=back.closest('.leaflet-popup-content');
    showPopupDateList(back.getAttribute('data-popup-group-id'),box);
    return;
  }

  const dd=ev.target.closest('[data-popup-date-key]');
  if(dd){
    ev.preventDefault();ev.stopPropagation();
    const box=dd.closest('.leaflet-popup-content');
    const gid=dd.getAttribute('data-popup-group-id');
    const g=(groups||[]).find(x=>String(x.group_id)===String(gid));
    if(g&&box)box.innerHTML=popupTimeList(g,dd.getAttribute('data-popup-date-key'));
    return;
  }
...
function v112_makePicoSummary(p){
  return {
    ...
    source:'pico_log',
    sid:String(p.sid || ''),
    gps_visit_id:String(p.gps_visit_id || p.visit_id || ''),
    pico_point_visit_id:String(p.pico_point_visit_id || ''),
...
async function v112_stage2FindTripForLogSync(p){
  const trips = await getAllTrips();
  ...
  // Stage2: gps_visit_candidates から確定した実釣visitは gps_visit_id を最優先にする。
  // point_visit_id / map_point_key は地図ピン主キーとして使わない。
  if(visitKey){
    for(const t of trips){
      if(String(t.gps_visit_id || '') === visitKey) return t;
      if(t.pico_summary && String(t.pico_summary.gps_visit_id || '') === visitKey) return t;
      if(Array.isArray(t.pico_logs) && t.pico_logs.some(l => String(l.gps_visit_id || '') === visitKey)) return t;
    }
  }

  // 旧互換: gps_visit_id が無い従来payloadは、これまで通り point_visit_id / map_point_key で更新する。
...
async function v112_findTripForLogSync(p){
  return await v112_stage2FindTripForLogSync(p);
}
...
function v112_makeTripFromLogSync(p){
  ...
  const visitKey = String(p.gps_visit_id || p.visit_id || '').trim();
  ...
  return {
    ...
    point_visit_id:visitKey ? '' : String(p.point_visit_id || ''),
    map_point_key:visitKey ? '' : String(p.map_point_key || p.point_visit_id || ''),
    pico_point_visit_id:String(p.pico_point_visit_id || ''),
    map_spot_id:String(p.map_spot_id || p.spot_id || ''),
    date_ms:v112_numOrNull(p.start_ms) || v112_numOrNull(p.visit_start_ms) || v112_numOrNull(p.first_recv_ms) || now,
    lat:Number.isFinite(gpsLat) ? gpsLat : 0,
    lng:Number.isFinite(gpsLng) ? gpsLng : 0,
    accuracy_m:v112_numOrNull(p.gps_acc_m || p.acc) || 0,
    location_time_ms:v112_numOrNull(p.visit_start_ms) || v112_numOrNull(p.gps_ms) || v112_numOrNull(p.start_ms) || now,
    lake_name:String(p.lake_name || ''),
    point_name:String(p.point_name || p.place_name || (visitKey ? 'Pico W実釣地点' : 'Pico Wログ地点')),
...
function v112_stage2MergeVisitPayload(parent, visit){
  const p = Object.assign({}, parent || {}, visit || {});
  p.__stage2_single_visit = true;
  p.sid = String((parent && parent.sid) || (visit && visit.sid) || '');
  ...
  for(const k of fallbackKeys){
    if((p[k] === undefined || p[k] === null || p[k] === '') && parent && parent[k] !== undefined){
      p[k] = parent[k];
    }
  }

  if(!p.gps_lat && visit && (visit.latest_lat || visit.lat)) p.gps_lat = visit.latest_lat || visit.lat;
  if(!p.gps_lng && visit && (visit.latest_lng || visit.lng)) p.gps_lng = visit.latest_lng || visit.lng;
  if(!p.gps_acc_m && visit && (visit.acc_m || visit.gps_acc_m)) p.gps_acc_m = visit.acc_m || visit.gps_acc_m;

  // GPS候補から確定したvisitでは、Pico Wのpoint_visit_idはログ区間IDとして保持し、
  // 地図ピン主キーには使わない。
  if(p.gps_visit_id || p.visit_id){
    p.pico_point_visit_id = String((visit && visit.pico_point_visit_id) || (parent && (parent.point_visit_id || parent.map_point_key || parent.pico_point_visit_id)) || '');
    p.point_visit_id = '';
    p.map_point_key = '';
  }
...
async function v112_applyLogSyncPayload(p){
  ...
  // Stage2:
  // gps_candidates があるpayloadだけ、実釣判定済み gps_visit_candidates を複数visit保存する。
  // gps_candidates が0件の環境では従来の1件同期へフォールバックし、既存運用を止めない。
  ...
  if(
    ...
    const visits = p.gps_visit_candidates.filter(v =>
      v &&
      (v.gps_visit_id || v.visit_id) &&
      (v.gps_lat || v.latest_lat || v.lat) &&
      (v.gps_lng || v.latest_lng || v.lng)
    );
    ...
    if(!visits.length){
      ...
      await refreshAll();
      v112_setLogSync('実釣なし: 保存なし','warn');
      return false;
    }

    let saved = 0;
    for(const v of visits){
      const one = v112_stage2MergeVisitPayload(p, v);
      if(await v112_applyLogSyncPayload(one)) saved++;
    }
    ...
    v112_setLogSync('実釣' + saved + '地点同期','good');
    return saved > 0;
  }
  ...
  if(visitKey){
    t.gps_visit_id = String(t.gps_visit_id || visitKey || '');
    t.pico_point_visit_id = String(t.pico_point_visit_id || p.pico_point_visit_id || '');
  }else{
  ...
  // 0mは登録しない。10m物理地点グループ側の過去履歴値とは混ぜない。
  const incomingDepth = v112_depthMaxText(
    p.fishfinder_depth_m,
    p.max_depth_m,
    p.fishfinder_m
  ...
  if(visitKey){
    t.pico_logs = t.pico_logs.filter(x => String(x.gps_visit_id || '') !== visitKey);
  }else{
  ...
  v112_setLogSync(visitKey ? '実釣地点同期済み' : '同期済み','good');
  return true;
}
