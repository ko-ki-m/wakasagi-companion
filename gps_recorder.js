(function(){
  ...
  const VERSION = 'gps_recorder_20260531s_no_acc_reject_gps_only_20260614a';
  ...
  function dbg(obj){
    ...
    debugEl.textContent = JSON.stringify(Object.assign({
      ...
      behavior: 'request_only_one_shot_named_log_no_acc_reject_gps_only',
      auto_timer: false,
      autostart: false,
      watchPosition: false,
  ...
  async function commitPosition(pos, source, requestMeta){
    ...
    dbg(Object.assign({
      saved: ok,
      note: '要求時だけ1回取得。定期取得・autostart・watchPositionはありません。accが悪くても保存拒否しません。Map履歴trip_recordsには書き込みません。'
    }, result));
