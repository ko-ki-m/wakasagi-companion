(function(){
  ...
  const VERSION = 'gps_recorder_20260527k_request_runner';
  ...
  function setStatus(text, cls){
    if(statusEl){ statusEl.textContent = text; statusEl.className = 'status ' + (cls || ''); }
  }
  ...
  function dbg(obj){
    if(debugEl){
      debugEl.textContent = JSON.stringify(Object.assign({
        ...
        behavior:'request_only_one_shot',
        auto_timer:false,
        autostart:false,
        watchPosition:false,
  ...
  function getLatestCandidate(){
    return new Promise(resolve=>{
      ...
      req.onsuccess = ev => { const cur=ev.target.result; resolve(cur ? cur.value : null); };
    ...
    return new Promise(resolve=>{
      ...
      try{
        ...
        req.onsuccess = ev => {
          ...
          if(!cur) return;
          const r = cur.value || {};
          if(String(r.sid || '') === String(sidTarget)){ cur.delete(); count++; }
          cur.continue();
        };
  ...
  async function clearCurrentSidCandidates(){
    ...
    if(!sid){ setStatus('sidなし。削除対象を特定できません。','bad'); dbg({error:'missing sid for delete'}); return; }
  ...
  async function commitPosition(pos, source, requestMeta){
    ...
    if(!sid && requestMeta && requestMeta.sid){ sid = String(requestMeta.sid || '').trim(); }
    if(sid) localStorage.setItem('wakasagi_last_sid', sid);

    if(!sid){ setStatus('sidなし。/logからsid付きで開く必要があります。','bad'); dbg({error:'missing sid'}); return {ok:false,error:'missing_sid'}; }
    if(!Number.isFinite(lat) || !Number.isFinite(lng)){ setStatus('GPS値が不正です','bad'); dbg({error:'invalid lat/lng'}); return {ok:false,error:'invalid_latlng'}; }
    if(Number.isFinite(acc) && acc > MAX_ACC_M){ setStatus('GPS精度不足: ' + acc.toFixed(1) + 'm','warn'); dbg({lat:lat.toFixed(7), lng:lng.toFixed(7), acc_m:acc.toFixed(1), rejected:'accuracy'}); return {ok:false,error:'accuracy',lat,lng,acc_m:acc}; }
    ...
    if(!latest || d === null || d >= MOVE_M){
      ...
      rec = {
        id:idFor(no), sid:sid || 'nosid', pico:pico, candidate_no:no,
        start_ms:now(), end_ms:now(),
        lat:lat.toFixed(7), lng:lng.toFixed(7), latest_lat:lat.toFixed(7), latest_lng:lng.toFixed(7),
      ...
      if(Number.isFinite(acc) && acc < prevBest){ rec.best_acc_m = acc.toFixed(1); rec.acc_m = acc.toFixed(1); }
    ...
    dbg(Object.assign({saved:ok, note:'要求時だけ1回取得。定期取得・autostart・watchPositionはありません。'}, result));
    return result;
  }
  ...
  async function sampleOnce(source, requestMeta){
    ...
    try{
      if(!db) db = await openDb();
      if(requestMeta && requestMeta.sid){ sid = String(requestMeta.sid || '').trim(); }
      if(!sid){ setStatus('sidなし。/logからsid付きで開く必要があります。','bad'); dbg({error:'missing sid'}); return {ok:false,error:'missing_sid'}; }
  ...
  function stop(){ setStatus('待機中。定期取得はありません。','warn'); dbg({stopped:true}); }
  ...
  async function handleRequestMessage(ev){
    ...
    if(!originOk){ dbg({rejected:'origin', origin:ev.origin, allowedPicoOrigin:allowedPicoOrigin}); return; }
    ...
    try{
      if(ev.source && ev.source.postMessage){ ev.source.postMessage(reply, ev.origin || '*'); }
    }catch(e){}
  }
  ...
  (async()=>{
    ...
    try{
      if(window.opener && window.opener.postMessage){
        window.opener.postMessage({type:'wakasagi:gps-recorder-ready', sid:sid, version:VERSION}, allowedPicoOrigin === '*' ? '*' : allowedPicoOrigin);
      }
    }catch(e){}
  })();
})();
